...
import { resolveTargetUser } from '../src/commands/target.js';
import combattiCommand from '../src/commands/rpg/combatti.js';
import rubaCommand from '../src/commands/rpg/ruba.js';
...
describe('target precedence (mention → quote → none)', () => {
  ...
  let caller: WaUserIdentity;
  let owner: WaUserIdentity;
  ...
  function buildDispatchers() {
    const rpgView = new RpgService(new PlayerRepository(db)) as RpgServiceView;
    ...
    const map = { combat, rob, add, rm } as Record<string, CommandDispatcher>;
  ...
  function identityFor(c: Case): WaUserIdentity {
    return c.ownerOnly ? owner : caller;
  }

  function actorIdentityId(c: Case): string {
    return identityService.fromJid(c.ownerOnly ? '1234@s.whatsapp.net' : '9000@s.whatsapp.net')!.userId;
  }

  function seedPlayers(rpgView: RpgServiceView, userIds: string[]) {
    for (const userId of userIds) rpgView.getOrCreatePlayer(userId);
  }
  ...
  describe('resolveTargetUser helper', () => {
    ...
    it('returns the mention when only a mention is present', () => {
      assert.equal(resolveTargetUser([mentionA] as never, null), mentionA);
    });

    it('returns the quoted sender when only a quote is present', () => {
      assert.equal(resolveTargetUser([] as never, quoteB), quoteB);
    });

    it('prefers the mention over the quote when both are present', () => {
      assert.equal(resolveTargetUser([mentionA] as never, quoteB), mentionA);
    });

    it('returns null when neither a mention nor a quote is present', () => {
      assert.equal(resolveTargetUser([] as never, null), null);
    });
  });
      ...
      describe(c.label, () => {
        ...
        it('mention wins over quoted sender (case 3)', async () => {
          ...
          const out = await run(
            ...
            c.text,
            identityFor(c),
            [makeMention(TARGET_PN, mentionTarget)],
            makeMention(OTHER_PN, quotedTarget),
          );
