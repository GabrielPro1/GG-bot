...
function emit(manager: SessionManager, userId: string, update: ConnectionState): void {
  const sock = manager.getSession(userId) as unknown as StubSocket;
  sock.ev.emit('connection.update', update);
}

interface RecordingChat {
  texts: string[];
  images: Buffer[];
}

function makeChat(): { chat: RecordingChat & { sendText: (t: string) => Promise<void>; sendQr: (b: Buffer, c: string) => Promise<void> } } {
  const record: RecordingChat = { texts: [], images: [] };
  const chat = {
    ...record,
    sendText: async (t: string) => {
      record.texts.push(t);
    },
    sendQr: async (b: Buffer) => {
...
function makeLinker(manager: SessionManager, timeoutMs = 60_000) {
...
test('start + onQr sends a PNG (never the raw QR as text)', async () => {
  ...
  emit(manager, '123', { connection: 'connecting', qr: 'RAW-QR-STRING' } satisfies ConnectionState);
  await sleep(5);
...
test('onOpen sends success text and settles the run', async () => {
  ...
  await linker.start('123', chat);
  emit(manager, '123', { connection: 'open' } satisfies ConnectionState);
...
test('start refuses when a session already exists for the user (no duplicate socket)', async () => {
  ...
  emit(manager, '555', { connection: 'open' } satisfies ConnectionState);
...
test('connection close before completion informs the user', async () => {
  ...
  emit(manager, '777', { connection: 'close' } satisfies ConnectionState);
