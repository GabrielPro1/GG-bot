...
import { AIService, disabledAiView } from '../lib/ai/ai.service.js';
...
describe('AI service', () => {
    ...
    it('creates a chat with per-user progressive ids (1, 2, ...)', () => {
        ...
        if (first.ok)
            assert.equal(first.chatNumber, 1);
        if (second.ok)
            assert.equal(second.chatNumber, 2);
        if (third.ok)
            assert.equal(third.chatNumber, 3);
    });
    it('keeps chat ids independent per user', () => {
        ...
        if (a1.ok)
            assert.equal(a1.chatNumber, 1);
        if (b1.ok)
            assert.equal(b1.chatNumber, 1);
        if (a2.ok)
            assert.equal(a2.chatNumber, 2);
    });
    ...
    it('sends the full reconstructed history to the provider', async () => {
        ...
        assert.deepEqual(lastCall.map((m) => m.role), ['user', 'model', 'user']);
        assert.deepEqual(lastCall.map((m) => m.text), ['primo messaggio', 'Ciao! Come posso aiutarti?', 'secondo messaggio']);
    });
    ...
    it('lists the user chats', () => {
        ...
        assert.equal(result.ok, true);
        if (result.ok) {
            assert.deepEqual(result.chats.map((c) => c.chatNumber), [1, 2]);
        }
    });
    ...
    it('deletes a chat and its messages', async () => {
        ...
        if (list.ok)
            assert.equal(list.chats.length, 0);
    });
...
describe('AI command integration via dispatcher', () => {
    function buildWithCommands() {
        ...
        const ai = new AIService(new AiRepository(db), new FakeProvider('Risposta AI'));
        const dispatcher = new CommandDispatcher(registry, {}, {}, ai);
        return { dispatcher };
    }
    async function run(dispatcher, text) {
        const replies = [];
