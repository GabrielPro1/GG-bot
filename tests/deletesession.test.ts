...
import { TelegramLinker } from '../src/telegram/link.js';
...
async function seedAuthFiles(authRoot: string, userId: string): Promise<string> {
...
test('deleteSession deletes the auth folder and closes the active socket', async () => {
  ...
  try {
    ...
    const sock = session.sock as unknown as StubSocket;
...
test('reconnect does not recreate a session after deleteSession', async () => {
  ...
  try {
    ...
    (session.sock as unknown as StubSocket).ev.emit('connection.update', {
      connection: 'close',
