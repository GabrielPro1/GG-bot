...
import { RpgService, dayKeyFromDate, selectMissionsForDay } from '../src/services/rpg/rpg.service.js';
...
import type {
  CombatRandomSource,
  HuntRandomSource,
} from '../src/services/rpg/types.js';
...
describe('RpgService', () => {
  ...
  it('deposit fails with insufficient wallet', () => {
    ...
    if (!result.ok) assert.equal(result.error, 'insufficient_wallet');
  });
  ...
  it('withdraw fails with insufficient bank', () => {
    ...
    assert.equal(result.ok, false);
    if (!result.ok) assert.equal(result.error, 'insufficient_bank');
  });
  ...
  it('spendWallet succeeds with sufficient balance', () => {
    ...
    assert.equal(result.ok, true);
    if (result.ok) assert.equal(result.walletCoins, 60);
  });
  ...
  it('addItem with unknown item returns error', () => {
    ...
    if (!res.ok) assert.equal(res.error, 'unknown_item');
  });
  ...
  it('equipItem fails when item not in inventory', () => {
    ...
    assert.equal(res.ok, false);
    if (!res.ok) assert.equal(res.error, 'not_in_inventory');
  });
  ...
  it('useItem fails when energy is full', () => {
    ...
    const res = rpg.useItem(uid, 'pozione');
    assert.equal(res.ok, false);
    if (!res.ok) assert.equal(res.error, 'energy_full');
  });
  ...
  it('consumeEnergy reduces energy', () => {
    ...
    assert.equal(res.ok, true);
    if (res.ok) assert.equal(res.energy, 70);
  });
  ...
  it('consumeEnergy fails when insufficient', () => {
    ...
    assert.equal(res.ok, false);
    if (!res.ok) assert.equal(res.error, 'insufficient_energy');
  });
  ...
  it('restoreEnergy increases energy up to max', () => {
    ...
    assert.equal(res.ok, true);
    if (res.ok) assert.equal(res.energy, 100); // capped at maxEnergy
  });
  ...
  it('startCombat sets combatCooldownUntil on attacker', () => {
    ...
    const now = 1000000;
    const deterministic: CombatRandomSource = {
  ...
  it('hunt sets huntCooldownUntil', () => {
    rpg.getOrCreatePlayer(uid);
    const now = 1000000;
    const deterministic: HuntRandomSource = {
      pickMonsterIndex: () => 0,
  ...
  it('startCombat rejects self_target', () => {
    ...
    assert.equal(res.ok, false);
    if (!res.ok) assert.equal(res.reason, 'self_target');
  });
  ...
  it('startCombat fails when attacker not found', () => {
    ...
    assert.equal(res.ok, false);
    if (!res.ok) assert.equal(res.reason, 'attacker_not_found');
  });
  ...
  it('startCombat fails when defender not found', () => {
    ...
    assert.equal(res.ok, false);
    if (!res.ok) assert.equal(res.reason, 'target_not_found');
  });
  ...
  it('purchaseItem fails with insufficient wallet', () => {
    ...
    assert.equal(res.ok, false);
    if (!res.ok) assert.equal(res.error, 'insufficient_wallet');
  });
