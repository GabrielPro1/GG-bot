...
import type { AiRequestMessage } from '../src/services/ai/types.js';
import { handleAiModeMessage } from '../src/services/ai/ai-mode.js';
...
import esciCommand from '../src/commands/ai/esci-chat.js';
...
class FakeProvider implements AiProvider {
  ...
  public reply = 'Ciao! Come posso aiutarti?';
...
function buildDb(): { db: Database.Database } {
...
describe('AI mode', () => {
  ...
  it('AI mode and history clear correctly after /esci chat while chat remains in DB', async () => {
    ...
    if (list.ok) assert.equal(list.chats.length, 1);
    const chat = repo.findChat('u1', 1)!;
  ...
  it('a raw /esci chat command via dispatcher deactivates AI mode and replies', async () => {
    ...
    registry.register(esciCommand as Command);
  ...
  it('/ai crea chat via dispatcher activates AI mode and /ping still works', async () => {
    ...
    registry.register(pingCommand as Command);
