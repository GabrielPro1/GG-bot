...
function resolveTargetDirectory() {
    ...
    return {
        dir: path.join(projectRoot, 'plugins'),
        extension: '.js',
    };
}
function isRelevantFile(fileName, extension) {
    if (!fileName.endsWith(extension))
        return false;
    const base = fileName.slice(0, -extension.length);
    if (INFRA_BASENAMES.has(base))
        return false;
    const lower = fileName.toLowerCase();
    if (IGNORED_PREFIXES.some((prefix) => fileName.startsWith(prefix)))
        return false;
...
export class CommandLoader {
    ...
    async loadFile(filePath) {
        ...
        if (previous && previous.name !== nextKey && this.registry.get(nextKey)) {
            console.error(`${LOG_PREFIX} cannot rename "${previous.command.name}" to "${candidate.name}" ` +
        ...
        if (!registration.ok) {
            console.error(`${LOG_PREFIX} failed to register "${candidate.name}" (${path.basename(absPath)}): ${registration.error}`);
            if (previous) {
                ...
                if (restored.ok) {
                    ...
                    console.error(`${LOG_PREFIX} rollback failed for "${previous.command.name}": ${restored.error}`);
                }
            }
            return;
        }
    ...
    async loadAll() {
        ...
        for (const file of files) {
            try {
                ...
                console.error(`${LOG_PREFIX} failed to load ${path.basename(file)}:`, error instanceof Error ? error.message : error);
            }
        }
    }
    startWatcher() {
    ...
    async refresh(absPath) {
        try {
            if (!isRelevantFile(path.basename(absPath), this.extension))
                return;
            ...
            console.error(`${LOG_PREFIX} error while refreshing ${path.basename(absPath)}:`, error instanceof Error ? error.message : error);
        }
    }
}
