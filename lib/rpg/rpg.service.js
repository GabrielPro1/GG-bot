...
import { isEquippable } from './items/types.js';
...
function computeEffectiveStats(player) {
    ...
    for (const itemId of Object.values(player.equipment)) {
        ...
        if (item.effect.kind === 'attack')
            attackBonus += item.effect.amount;
        else if (item.effect.kind === 'defense')
            defenseBonus += item.effect.amount;
        else if (item.effect.kind === 'luck')
            luckBonus += item.effect.amount;
    }
    return {
...
export class RpgService {
    ...
    getLeaderboardByLevel(limit = DEFAULT_LEADERBOARD_LIMIT) {
    ...
    getLeaderboardByWins(limit = DEFAULT_LEADERBOARD_LIMIT) {
        return this.repo.getAllPlayers()
    ...
    getLeaderboardByWealth(limit = DEFAULT_LEADERBOARD_LIMIT) {
        return this.repo.getAllPlayers()
    ...
    withdraw(userId, amount) {
        if (!isValidAmount(amount))
            return { ok: false, error: 'invalid_amount' };
        const player = this.getOrCreatePlayer(userId);
        if (amount > player.bankCoins)
            return { ok: false, error: 'insufficient_bank' };
        player.bankCoins -= amount;
        player.walletCoins += amount;
        this.repo.savePlayer(player);
        return { ok: true, walletCoins: player.walletCoins, bankCoins: player.bankCoins };
    }
    spendWallet(userId, amount) {
        if (!isValidAmount(amount))
            return { ok: false, error: 'invalid_amount' };
        const player = this.getOrCreatePlayer(userId);
        if (amount > player.walletCoins)
    ...
    equipItem(userId, itemId) {
        ...
        if (!slot || !isEquippable(item))
            return { ok: false, error: 'not_equippable', item };
        const player = this.getOrCreatePlayer(userId);
    ...
    useItem(userId, itemId) {
        ...
        if (item.type !== 'consumable')
            return { ok: false, error: 'not_consumable', item };
        const player = this.getOrCreatePlayer(userId);
    ...
    rob(thiefUserId, victimUserId, options = {}) {
        return this.repo.transaction(() => {
            ...
            const percentage = Math.floor(rng() * (ROB_MAX_PERCENTAGE - ROB_MIN_PERCENTAGE + 1)) + ROB_MIN_PERCENTAGE;
            ...
            if (rng() >= ROB_SUCCESS_CHANCE) {
                const penalty = { until: now + ROB_PRISON_MS, jailed: true };
    ...
    startCombat(attackerUserId, defenderUserId, options = {}) {
        return this.repo.transaction(() => {
            const now = options.now ?? Date.now();
    ...
    grantHuntLoot(userId, monster, randomSource) {
        const aggregated = new Map();
        for (const entry of monster.loot) {
            const resolved = resolveLootEntry(monster.id, entry);
            if (!resolved)
                continue;
            if (!randomSource.rollLootDrop(resolved.dropChance))
                continue;
    ...
    trackMissionProgress(userId, goalType, now = Date.now()) {
    ...
    getDailyMissions(userId, now = Date.now()) {
        const dayKey = dayKeyFromDate(new Date(now));
        const player = this.getOrCreatePlayer(userId);
        return selectMissionsForDay(dayKey).map((mission) => {
            const state = player.missions[mission.id];
            const progress = state && state.dayKey === dayKey ? Math.min(mission.target, state.progress) : 0;
    ...
    claimMission(userId, missionId, now = Date.now()) {
        return this.repo.transaction(() => {
            const dayKey = dayKeyFromDate(new Date(now));
            ...
            const state = existing && existing.dayKey === dayKey ? existing : { dayKey, progress: 0, claimed: false };
            if (state.claimed)
...
function createDefaultCombatRandomSource(attackerLuck, defenderLuck) {
    return {
        rollAttackerCritical: () => Math.random() * 100 < attackerLuck,
        rollDefenderCritical: () => Math.random() * 100 < defenderLuck,
    };
}
function createDefaultHuntRandomSource(playerLuck) {
    return {
...
export function selectMissionsForDay(dayKey) {
    let checksum = 0;
...
function resolveLootEntry(monsterId, entry) {
    ...
    if (!Number.isFinite(entry.dropChance) || entry.dropChance < 0 || entry.dropChance > 100) {
        console.warn(`[rpg] Loot ignorato (${monsterId}): dropChance non valida ${entry.dropChance} per "${entry.itemId}"`);
        return null;
    }
    const minQuantity = entry.minQuantity ?? 1;
    const maxQuantity = entry.maxQuantity ?? minQuantity;
    if (!isValidAmount(minQuantity) ||
        minQuantity < 1 ||
        !isValidAmount(maxQuantity) ||
        maxQuantity < minQuantity) {
        console.warn(`[rpg] Loot ignorato (${monsterId}): quantità non valide [${minQuantity}, ${maxQuantity}] per "${entry.itemId}"`);
        return null;
    }
