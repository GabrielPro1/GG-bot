...
export function registerHandlers(bot, linker) {
    bot.command('start', (ctx) => {
        void ctx.reply('🤖 Benvenuto! Questo bot ti permetterà di collegare il tuo numero WhatsApp.', {
            reply_markup: {
                inline_keyboard: [
    ...
    bot.command('deletesession', async (ctx) => {
        ...
        if (deleted) {
            await ctx.reply('🗑️ Sessione WhatsApp eliminata correttamente.\nOra puoi collegare nuovamente il tuo numero.');
        }
        else {
